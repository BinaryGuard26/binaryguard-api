...
async function findTenantByEmail(email: string): Promise<{ tenantId: string } | null> {
...
app.post("/api/otp/request", async (req: Request, res: Response) => {
  try {
    ...
    const { error } = await supabase.from("otp_codes").insert({
      ...
      email,
      purpose,
      code_hash: hashOtp(email, purpose, code),
    ...
    return res.status(500).json({ ok: false, message: "Unable to send OTP." });
  }
});
...
app.post("/api/otp/verify", async (req: Request, res: Response) => {
  try {
    ...
    const valid = submittedHash.length === otpRow.code_hash.length &&
      crypto.timingSafeEqual(Buffer.from(submittedHash, "hex"), Buffer.from(otpRow.code_hash, "hex"));

    if (!valid) {
      const nextAttempts = otpRow.attempt_count + 1;
      const updatePayload: { attempt_count: number; used_at?: string } = { attempt_count: nextAttempts };
    ...
    if (purpose === "registration") {
      await supabase
        ...
        .update({ status: "otp_verified", otp_verified_at: nowIso() })
        .eq("corporate_email", email)
        .in("status", ["submitted", "otp_pending"]);
    }

    return res.json({ ok: true, message: "OTP verified successfully.", tenant_id: otpRow.tenant_id });
  } catch (error) {
